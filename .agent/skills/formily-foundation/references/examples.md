...
6. [分步表单（Steps + 多 YssFormily）](#6-分步表单steps--多-yssformily)
...
```vue
<template>
  <YssFormily :schema="schema" :initial-values="{ needCompany: false }" />
</template>
...
**特点**：根据 `mode`（0=新增/1=编辑/2=查看）切换表单状态；`mode=2` 时自动进入 Descriptions 查看态，描述列表参数通过 `detail-options` 配置。
  ...
  <YssFormily :schema="schema">
    ...
    <template #userSelector="{ value, onChange, field, values }">
      <div style="display: flex; gap: 8px">
        ...
        <span style="color: var(--text-color-secondary, rgba(0, 0, 0, 0.65))">
          当前分类: {{ values?.category || '无' }}
        </span>
      </div>
    </template>
  </YssFormily>
...
## 6. 分步表单（Steps + 多 YssFormily）

**特点**：业务层默认采用 Ant Design `Steps` + 多个 `YssFormily`，每一步单独校验并共享状态，不依赖 `@formily/antdv FormStep`。
...
<template>
  <div style="padding: 24px;">
    ...
    <div style="margin-top: 16px; text-align: right">
      <a-button :disabled="currentStep === 0" @click="back">上一步</a-button>
      <a-button
        v-if="currentStep < 2"
        type="primary"
...
const step1Ref = ref<any>();
const step2Ref = ref<any>();
...
const next = async () => {
  if (currentStep.value === 0) {
    const ok = await step1Ref.value?.submit?.();
    if (ok === false) return;
  }
  if (currentStep.value === 1) {
    const ok = await step2Ref.value?.submit?.();
    if (ok === false) return;
  }
  currentStep.value += 1;
};
